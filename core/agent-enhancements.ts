...
export default function agentEnhancements(pi: ExtensionAPI): void {
	...
	// ─── Register cost tracking command ────────────────────────────────────
	pi.registerCommand("agent-costs", {
		description: "Show agent cost tracking",
		handler: (_args, ctx) => {
			const costs = loadCosts();
			if (costs.length === 0) {
				ctx.ui.notify("No cost data recorded yet.", "info");
				return;
			}

			const totalTokens = costs.reduce((sum, c) => sum + c.tokens, 0);
			const recentCosts = costs.slice(-10);

			const lines = [
				`Total tokens used: ${totalTokens.toLocaleString()}`,
				`Sessions recorded: ${costs.length}`,
				"",
				"Recent usage:",
			];

			for (const cost of recentCosts) {
				lines.push(`  ${cost.agent}: ${cost.tokens.toLocaleString()} tokens`);
			}
	...
	pi.registerCommand("agent-cache", {
		description: "Show or clear agent result cache",
		handler: (_args, ctx) => {
			try {
				if (!existsSync(CACHE_DIR)) {
					ctx.ui.notify("No cache entries.", "info");
					return;
				}
				...
				ctx.ui.notify(`Cache entries: ${files.length}\nLocation: ${CACHE_DIR}`, "info");
			} catch {
				ctx.ui.notify("No cache entries.", "info");
			}
		},
